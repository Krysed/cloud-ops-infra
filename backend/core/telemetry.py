...
def configure_telemetry(app_name: str = "fastapi-backend"):
    ...
    )  # Creates /metrics endpoint for Mimir to scrape
...
def init_http_metrics():
    ...
    login_attempts_total = meter.create_counter(
        name="login_attempts_total", description="Total login attempts", unit="1"
    )

    postings_created_total = meter.create_counter(
        name="postings_created_total", description="Total postings created", unit="1"
    )
...
def instrument_app(app):
    ...
    print(
        "OpenTelemetry instrumentation enabled with HTTP metrics, sending data to LGTM stack"
    )
