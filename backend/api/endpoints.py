...
async def create_user_account(
    ...
    try:
        ...
        if username.lower() == email.lower():
            logger.info(
                f"Registration failed: Username and email cannot be the same - {username}"
            )
            record_user_registration("error")
            return RedirectResponse(
                url="/register.html?error=username_email_same", status_code=303
            )
        ...
        if get_user_by_email(email):
            ...
            record_user_registration("error")
            return RedirectResponse(
                url="/register.html?error=email_taken", status_code=303
            )
        ...
        if get_user_by_username(username):
            ...
            record_user_registration("error")
            return RedirectResponse(
                url="/register.html?error=username_taken", status_code=303
            )

        user_id = create_user(
            name, surname, username, email, hashed_password=hash_password(password)
        )
        ...
        return RedirectResponse(
            url="/login.html?success=account_created", status_code=303
        )
        ...
        record_user_registration("error")
        return RedirectResponse(
            url="/register.html?error=server_error", status_code=303
        )
...
async def create_posting(
    ...
    record_posting_created()
    return RedirectResponse(
        url="/my-postings.html?success=posting_created", status_code=303
    )
...
async def update_posting(
    ...
    try:
        ...
        if not posting or posting["user_id"] != user_id:
            return RedirectResponse(
                url="/my-postings.html?error=access_denied", status_code=303
            )

        success = update_posting_in_db(
            posting_id, title, category, post_description, status
        )
        if not success:
            return RedirectResponse(
                url="/my-postings.html?error=update_failed", status_code=303
            )

        return RedirectResponse(
            url="/my-postings.html?success=posting_updated", status_code=303
        )
    except Exception:
        return RedirectResponse(
            url="/my-postings.html?error=update_failed", status_code=303
...
async def delete_posting(posting_id: int, request: Request):
    ...
    if posting["user_id"] != user_id:
        raise HTTPException(
            status_code=403,
            detail="Access denied - you can only delete your own postings",
        )
...
async def get_my_postings(request: Request):
    ...
    try:
        ...
        for posting in postings:
            ...
            posting["formatted_date"] = (
                posting["created_at"].strftime("%m/%d/%Y")
        ...
        raise HTTPException(
            status_code=500, detail="Failed to fetch your postings"
        ) from e
...
async def apply(
    ...
    logger.info(
        f"Application received for posting {posting_id}, message: {message[:50] if message else 'None'}"
    )
    ...
    try:
        ...
        if not posting:
            return RedirectResponse(
                url="/my-postings.html?error=posting_not_found", status_code=303
            )
        ...
        if not result["success"]:
            ...
            logger.warning(
                f"Application failed for posting {posting_id}, user {user_id}: {result['error']}"
            )
            record_application_submitted("failure")
            return RedirectResponse(
                url=f"/posting-detail.html?hash={posting_hash}&error={error_param}",
                status_code=303,
            )
        ...
        return RedirectResponse(
            url=f"/posting-detail.html?hash={posting_hash}&success=application_submitted",
            status_code=303,
        )
        ...
        record_application_submitted("failure")
        return RedirectResponse(
            url="/my-postings.html?error=application_failed", status_code=303
        )
...
async def view_posting(posting_hash: str, request: Request):
    ...
    if is_authenticated:
        ...
        if not is_owner:
            has_applied = check_user_application_exists(
                session_data["user_id"], posting_with_stats["id"]
            )
...
async def get_posting_analytics_endpoint(posting_id: int, request: Request):
    ...
    if not analytics:
        raise HTTPException(
            status_code=404, detail="Posting not found or access denied"
        )
...
async def data_view_page(request: Request):
    ...
    for posting in postings:
        posting["is_own"] = (
            posting["user_id"] == current_user_id if current_user_id else False
        )
...
async def view_posting_page(posting_id: int, request: Request):
    ...
    return RedirectResponse(
        url=f"/posting-detail.html?id={posting_id}", status_code=302
    )
...
async def login(request: Request, email: str = Form(...), password: str = Form(...)):
    try:
        ...
        if result is None:
            ...
            return RedirectResponse(
                url="/login.html?error=invalid_credentials", status_code=303
            )
...
async def auth_status(request: Request):
    ...
    if session_data:
        return JSONResponse(content={"authenticated": True, "user": session_data})
    else:
...
async def get_navigation(request: Request):
        ...
        nav_items = [
            {
                "url": "/data-view.html",
                "text": "All Postings",
                "path_check": "/data-view",
            },
            {
                "url": "/create-posting.html",
                "text": "Create Posting",
                "path_check": "/create-posting",
            },
            {
                "url": "/my-postings.html",
                "text": "My Postings",
                "path_check": "/my-postings",
            },
    ...
    return JSONResponse(
        content={"authenticated": bool(session_data), "nav_items": nav_items}
    )
